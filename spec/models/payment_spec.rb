...
RSpec.describe Payment, type: :model do
  ...
  context '.voucher_validation' do
    it 'e retorna um cupom de desconto válido' do
      ...
      order = Order.create!(id: 1, status: :insurance_approved, contract_period: 9, equipment:,
                            client:, insurance_name: insurance.insurance_name, package_name: insurance.name,
      ...
      fake_response = double('faraday_response', success?: true, body: json_data)
      allow(Faraday).to receive(:get).with("http://apipagamentos.com.br/api/v1/promos/#{voucher}/#{voucher_params}").and_return(fake_response)


      order.voucher_validation
      result_voucher = order.voucher_code
      result_discount = order.voucher_price
    ...
    it 'e retorna um cupom de desconto inválido' do
      ...
      fake_response = double('faraday_response', success?: true, body: json_data)
      allow(Faraday).to receive(:get).with("http://apipagamentos/api/v1/promos/#{voucher}/#{voucher_params}").and_return(fake_response)

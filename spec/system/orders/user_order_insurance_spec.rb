...
describe 'Cliente compra pacote de seguro' do
  ...
  it 'com sucesso' do
    ...
    equipment = Equipment.create!(client:, name: 'iphone 11', brand: 'Apple', equipment_price: 1000, purchase_date: '01/11/2022',
                                  ...
                                  photos: [fixture_file_upload('spec/support/photo_1.png'),
                                           fixture_file_upload('spec/support/photo_2.jpg')])
    insurance = Insurance.new(id: 2, name: 'Premium', max_period: 24, min_period: 6,
                              insurance_company_id: 1, insurance_name: 'Seguradora 45', price: 10.00,
                              product_category_id: 1, product_category: 'Celular', product_model: 'iphone 11')
    allow(Insurance).to receive(:find).with('2').and_return(insurance)
    ...
    params =  { policy: { client_name: client.name, client_registration_number: client.cpf, 
      ...
      package_id: insurance.id, insurance_company_id: insurance.insurance_company_id, equipment_id: equipment.id} }
  ...
  it 'e as APIs estão fora do ar' do
    client = Client.create!(name: 'Ana Lima', email: 'ana@gmail.com', password: '12345678', cpf: '21234567890',
                            address: 'Rua Dr Nogueira Martins, 680', city: 'São Paulo', state: 'SP',
                            birth_date: '29/10/1997')
    equipment = Equipment.create!(client:, name: 'iphone 11', brand: 'Apple', equipment_price: 1000, purchase_date: '01/11/2022',
    ...
    policy_fake_response = double('faraday_response', success?: false)
    params =  { policy: { client_name: client.name, client_registration_number: client.cpf, 
      client_email: client.email, policy_period: 7, order_id: 1,
      package_id: insurance.id, insurance_company_id: insurance.insurance_company_id, equipment_id: equipment.id} }
    allow(Faraday).to receive(:post)
    ...
    expect(page).not_to have_content 'Por favor verifique os erros abaixo'
    expect(page).not_to have_content 'Período de contratação não pode ficar em branco'
    expect(page).not_to have_content 'Dispositivo é obrigatório(a)'
